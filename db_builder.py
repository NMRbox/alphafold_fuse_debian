...
def create_or_update_sqlite(args: argparse.Namespace) -> None:
    ...
    with sqlite3.connect(args.sqlite_location) as sqlite_conn:
        ...
        if args.rebuild_entries:
            ...
            cursor.execute('CREATE UNIQUE INDEX uni_index ON files_tmp(uniprot_id, version);')
            # print('Building UniProt version index...')
            # cursor.execute('DROP INDEX IF EXISTS version_index;')
            # cursor.execute('CREATE UNIQUE INDEX version_index ON files_tmp(version, uniprot_id);')
